...
def get_firmware_revision(haddr):
    ...
    return subprocess.check_output(
        caen_module.cmdvme + " -wr " + format(haddr + address, "#x"),
        shell=True,
        executable="/bin/bash",
    )
...
class v1290(caen_module.caen_module):
    ...
    def init(self):
        if self.in_use:
            ...
            read_micro_register(self.haddr)  # Read value
            print(
                "Width, offset, extra search window, reject margin, trig time sub (1..enabled)"
            )
