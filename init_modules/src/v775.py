...
def get_firmware_rivision(haddr) :
    ...
    if debug :
        print(caen_module.cmdvme+" -wr "+dec2hex(haddr+address_firmware_revision), "(Getting Firmware Rivision)")
    return subprocess.check_output(caen_module.cmdvme+" -wr "+format(haddr+address_firmware_revision, "#x"), shell=True, executable="/bin/bash")

def set_full_scale_range(haddr, val) :
    address = 0x1060
    if debug :
        print(caen_module.cmdvme+" -ww "+ dec2hex(haddr+address) +" "+  dec2hex(val), "(Full Scale Range)")
...
def set_geometry(haddr, geo) :
    ...
    if debug :
        print(caen_module.cmdvme+" -ww "+ dec2hex(haddr+address_set_geometry) + " " +  dec2hex(geo), "(Setting geo)")
    subprocess.call(caen_module.cmdvme + " -ww " + format(haddr+address_set_geometry, "#x") + " " +  dec2hex(geo), shell=True, executable="/bin/bash")
...
def set_zero_supression(haddr, thresholds) :
    ...
    for ch, th in enumerate(thresholds) :
        if debug :
            print(caen_module.cmdvme+" -ww "+ dec2hex(haddr+address+2*ch) +" "+  dec2hex(th), "(Threshold)")
        subprocess.call(caen_module.cmdvme+" -ww "+ dec2hex(haddr+address+2*ch) +" "+  dec2hex(th), shell=True, executable="/bin/bash")
