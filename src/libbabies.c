...
void init_shm(void){
  sock_shm = mkudpsend(ANRCVPORT+sid, &caddr,  hostname);
  printf("\x1b[33mdebuginfo : init_shm() sock_shm = %d \x1b[39m\n", sock_shm);
}

void sendto_babian(char *dpt, int len){


//  RIDFHD hd;
//  RIDFRHD rhd;
//  char *data = (char *)malloc(len+sizeof(hd));
//  memset(data,0,sizeof(data));

  //'printf("\x1b[33mdebuginfo : sendto_babian() sending data, sock_shm=%d, len=%d \x1b[39m\n", sock_shm, len);
  unsigned short sval;
//  memcpy((char *)&hd, dpt, sizeof(hd));

//  printf("%04d\n", hd);
//  rhd = ridf_dechd(hd);

//  ridf_dechd`  switch(rhd.classid) {
//  ridf_dechd`    case RIDF_EA_BLOCK :
//  ridf_dechd`    case RIDF_EF_BLOCK :
//  ridf_dechd`    case RIDF_EAEF_BLOCK :
//  ridf_dechd`      printf("EA or EF Block\n");
//  ridf_dechd`      //memcpy(data,(char *)&hd);
//  ridf_dechd`      break;
//  ridf_dechd`  }


  sendto(sock_shm, dpt, len, 0, (struct sockaddr *)&caddr, sizeof(caddr));

  int i;
  for(i=0; i<64; i+=2){
    if(i%16 == 0){printf("\n");}
    memcpy((char*)&sval, dpt+i, sizeof(sval));
    printf("%04x ", sval);
  }
}
...
void babies_main(void) {
    ...
    while (1) {
        FD_ZERO(&fdset);
        //    printf("debuginfo : babies_main() FD_ZERO\n");
        FD_SET(comfd, &fdset);
        //    printf("debuginfo : babies_main() FD_SET %d\n", comfd);
        FD_SET(pfd[0], &fdset);
        //    printf("debuginfo : babies_main() FD_SET %d\n", pfd[0]);
        if (select(sn + 1, &fdset, NULL, NULL, NULL) != 0) {
            //    printf("debuginfo : babies_main() loop 0\n");
        ...
        //    printf("debuginfo : babies_main() End of loop.\n");
    }
}
...
int babies_end_block(void) {
    ...
    int len;
    memcpy((char *)&len, (char *)datapt[idx], sizeof(len));
    len = RIDF_SZ(len) * 2; // get buff length from data
    //  sendto_babian((char *)datapt[idx], len);
