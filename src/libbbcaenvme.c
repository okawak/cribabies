...
int vme_check_interrupt(void) {
    ...
    //       printf("OR down in vme_check_interrupt(): before CAENVME_IACKCycle\n");
    //       exit(1);
    // }
    if (msk && irqmask) {
        ret = 1;
        // HY 2021.12.13
        // Do you really need this error code? Omitted.
        //  error_code = CAENVME_IACKCycle(BHandle, intmsk, &vect, cvD32);

        // printf("CAENVME_IACKCycle( BHandle=%d, intmask=0x%02x, vect=%d, cvD32=0x%02x) = %d\n", BHandle, intmsk, vect, cvD32, error_code);
        if (vme_v2718_chk_output_register()) {
            printf("OR down in vme_check_interrupt(): after CAENVME_IACKCycle\n");
        }
    }
    return ret;
}
...
void vme_v2718_output_pulse(unsigned short mask) {
    ...
    //  //CAENVME_WriteRegister(BHandle,cvOutRegSet,mask);
    //  CAENVME_SetOutputRegister(BHandle,mask);
    // // usleep(1000);
    //  CAENVME_ClearOutputRegister(BHandle,mask);
    //  CAENVME_ReadRegister(BHandle,cvOutRegSet,&data);
    //  printf("CAENVME_ReadRegister: %04x\n",data);
